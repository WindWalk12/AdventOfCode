...
let cardsRank = ['J', '2', '3', '4', '5', '6', '7', '8', '9', 'T', 'Q', 'K', 'A']
...
let findStrength = (hand) => {
    ...
    Object.keys(cardsMap).forEach(key => {
        if (cardsMap[key] > higestOccurence && key != 'J') {
            higestOccurence = cardsMap[key]
            mapKey = key 
        }
    })
    if (typeof cardsMap['J'] !== "undefined") {
        higestOccurence += cardsMap['J']
        cardsMap[mapKey] += cardsMap['J']
    }
...
// hands.forEach(hand => {
//     console.log(hand.cards + " " + hand.strengh);
// })